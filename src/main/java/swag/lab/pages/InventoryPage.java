...
public class InventoryPage {
	...
	// item name =Test.allTheThings() T-Shirt (Red) , cart =
	...
	public boolean isProductListDisplayed() {
		if (items != null && !items.isEmpty()) {
			// Optional: You can iterate through the list to ensure at least one is
			// displayed
	 ...
	 * 
	 * // Locate elements inside item WebElement name =
	 * item.findElement(By.cssSelector(".inventory_item_name")); WebElement desc =
	 * item.findElement(By.cssSelector(".inventory_item_desc")); WebElement price =
	 * item.findElement(By.cssSelector(".inventory_item_price")); WebElement button
...
//	        }
	...
	public int getCartCount() {
		...
		try {
			...
			return Integer.parseInt(badgeCount.trim());
...
//	}
	...
	public void addMultipleProducts(int count) {
		...
		while (added < count) {
			...
			System.out.println("Clicked product = " + added);
	...
	// Get cart count // Not using in stepdefinition file
	public int getCartCount_() {
		try {
...
//	        }
	...
	public void removeProduct(String productName) {
		...
		List<WebElement> items = wait
				.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.className("inventory_item")));
	...
	public void removeProductByNameIndustrySolution(String productName) {
		...
		for (WebElement item : items) {
			...
			if (name.equalsIgnoreCase(productName)) {
				...
				wait.until(driver -> getCartCount() < before);
	...
	public void addProduct(String name) {
		for (WebElement item : items) {
			String itemName = item.findElement(org.openqa.selenium.By.cssSelector(".inventory_item_name")).getText();
	...
	public boolean hasBrokenImages() {
		for (WebElement item : items) {
			...
			// browser-side check
			Boolean loaded = (Boolean) js.executeScript("return arguments[0].complete && arguments[0].naturalWidth > 0",
					img);
